# Output values for the PowerTag module

output "instance_ids" {
  description = "IDs of the created EC2 instances"
  value       = aws_instance.powertag_instances[*].id
}

output "instance_public_ips" {
  description = "Public IPs of the created EC2 instances"
  value       = aws_instance.powertag_instances[*].public_ip
}

output "instance_private_ips" {
  description = "Private IPs of the created EC2 instances"
  value       = aws_instance.powertag_instances[*].private_ip
}

output "instance_arns" {
  description = "ARNs of the created EC2 instances"
  value       = aws_instance.powertag_instances[*].arn
}
...
output "cost_optimization_summary" {
  ...
  value = {
    ...
    daily_runtime_hours = calculate_daily_runtime()
    estimated_monthly_savings_percent = calculate_savings_percent()
  }
}
...
# Function to calculate daily runtime
function "calculate_daily_runtime" {
  result = local.daily_runtime_hours
}

# Function to calculate estimated savings percentage
function "calculate_savings_percent" {
  result = var.power_tag_days_active == "Mon-Fri" ? 
